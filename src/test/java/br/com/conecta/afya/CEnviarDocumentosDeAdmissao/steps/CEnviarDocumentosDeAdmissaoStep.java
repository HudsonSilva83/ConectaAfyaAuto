...
public class CEnviarDocumentosDeAdmissaoStep extends BaseTest {
	...
	static ColaboradorDAO etapaColaborador = new ColaboradorDAO();
	static EnviarDocumentosDAO etapaDoc = new EnviarDocumentosDAO();

	@Before
	public void setCenario(Scenario cen) throws SQLException {
		// feature = cen.getId().split(";")[0];
		cenario = cen.getName();
		etapaColaborador.cadastrarColaborador();
	...
	public static void finalizaCenario() throws SQLException {
		...
		etapaDoc.deletarDocumento();
		etapaColaborador.deletarColaborador();
	...
	public void o_app_apresenta_a_tela_com_o_numero_do_pis(String numeroPIS) {
		...
		// tem algo aqui
		// Assert.assertEquals(numeroPIS,
		// enviarDocPage.obterNumeroPISTela(numeroPIS));
		enviarDocPage.scroll(0.6, 0.1);
